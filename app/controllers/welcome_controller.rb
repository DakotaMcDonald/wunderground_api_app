class WelcomeController < ApplicationController
  def test
   w_response = HTTParty.get("http://api.wunderground.com/api/#{ENV['underground_api_key']}/conditions/q/CA/San_Francisco.json")
  ...
  def index 
  	@states = %w(HI AK CA OR WA ID UT NV AZ NM CO WY MT ND SD NB KS OK TX LA AR MO IA MN WI IL IN MI OH KY TN MS AL GA FL SC NC VA WV DE MD PA NY 
  	...
  	@forecast_link = w_response['current_observation']['forecast_url']
  	@feels_like = w_response['current_observation']['feelslike_f']	
  else w_response['response']['error']['description']
  	end
  	end
  end
end
